...
{
    const CONVERT_PATH = 'public/asset/cache';
    public $config;
    public $cache = true;
    ...
    public function getConvertPath($path, $baseDir = null)
    {
        ...
        if (is_numeric(strpos($path, $asset))) {
            // /project_xxx/public/asset/xxxx -> array('/project','xxxx')
    ...
    public function getConvertUrl($path, $width = null, $height = null, $opacity = 127, $baseDir = null, $wm = true)
    {
        ...
        if (strpos($path, ':')=='') {
            ...
            //create path dir
            if (!is_file($cacheFile) || !$this->cache) {
    ...
    public function make($srcImgPath, $targetImgPath, $targetW = null, $targetH = null, $a, $wm = true, $r = 255, $g = 255, $b = 255)
    {
     ...
     public function getTmpConvertUrl($_FILES_name, $width = null, $height = null, $opacity = 0)
     {
        ...
        copy($_FILES[$_FILES_name]['tmp_name'], ROOT . DS . $path);
     ...
     {
         $fileName = $name;
         copy(ROOT . DS . 'asset/internal/tmp/' . $fileName, ROOT . DS . 'asset/internal/' . $path);
     }

     public function clearCache()
     {
         Lib_IoUtils::instance()->clearDir(ROOT . DS . self::CONVERT_PATH);
     }
    ...
    function qrcode($str, $mode = 'png', $level = null, $size = 4, $margin = 4, $saveandprint = false)
    {
        ...
        switch ($mode) {
            ...
            case 'file':
                $key = md5($str);
                $path = '/public/asset/cache/file/qrcode/' . Lib_IoUtils::instance()->getMd5Dir($key) . '/' . $key . '.png';
                $file = ROOT . $path;
                if (!is_file($file)) {
                    $dir = dirname($file);
                    if (!is_dir($dir)) {
    ...
    function barcode($str, $size = 1, $font_size = 8, $code = null, $font_family = null, $filetype = null, $dpi = 72, $rotation = null)
    {
        if ($code == null) {
            $code = 'BCGcode128';
        }
        ...
        require_once 'barcodegen/image.php';
        $image = Lib_Barcodegen_image::instance();
        ...
        if (!empty($file['tmp_name'])) {
            ...
            if (in_array($fileExt, array('jpg', 'jpeg', 'gif', 'png'))) {
                if ($md5) {
                    ...
                    $path = '/public/static/asset/file/' . $ds . (empty($name) ? $md5str : urlencode($name) . '_' . $md5str) . '.' . $fileExt;
                    if (file_exists(ROOT . $path)) {//存在则直接返回
                        return $path;
                    }
                } else {
                    $path = '/public/static/asset/file/' . $ds . (empty($name) ? sfmd5_short(md5(date('Y-m-d'))) . sfrand_string(5) : urlencode($name)) . '.' . $fileExt;
                    while(file_exists(ROOT . $path)) {//重名
                        $path = str_replace('.' . $fileExt, sfrand_string(3) . '.' . $fileExt, $path);
                    }
                }
                Lib_IoUtils::instance()->createDir(dirname(ROOT . $path));
