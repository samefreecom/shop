...
*
*/
    static function Factory($name, $logger_name, $level) {
        $instance = new DebugOut($name, $logger_name, $level);
        return $instance;
    }
    static function getWriterSingleton($name, $logger_name, $level = ABC_DEBUG) {
        static $instances;
        if (!isset($instances)) {
            $instances = array();
        }
...
class ExcelFont {
    static function basicFontRecord() {
    ...
    static function getFontRecord(&$wb,$ptr) {
        $retval = array('size'     => 0,
            'script'   => XF_SCRIPT_NONE,
            'undeline' => XF_UNDERLINE_NONE,
            'italic'   => false,
    ...
    static function getUnicodeString(&$string,$offset,$length) {
        $bstring = "";
        $index   = $offset + 1;
        // start with low bits.
        for ($k = 0; $k < $length; $k++) {
